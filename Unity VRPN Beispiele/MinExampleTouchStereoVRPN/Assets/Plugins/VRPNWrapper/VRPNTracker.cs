...
public class VRPNTracker : MonoBehaviour {
	...
	private Quaternion resetOffset = Quaternion.identity;
    ...
    bool StartTracker () { 	
        if (VRPNManager.initialized)
        {
            ...
            VRPNTrackerStart("Rigid Body 1", (int)Derivation);
            num_trackers++;
    ...
    void Update () {
    	...
    	if (Derivation == Derivation_Type.None) {
    		...
    		if (DeviceToUnity != null)
    		{
				...
				if (SensorOffset != null)
    			{
					pos+=SensorOffset.transform.localPosition;
            ...
            //Debug.Log(pos);
				...
				{
					resetOffset = Quaternion.Inverse(quat);
				}
				quat*=resetOffset;
	...
	void GetLatestPosQuat()
    {
        ...
        VRPNTrackerPosReport("Rigid Body 1", repsPtr[0], IntPtr.Zero, SensorNumber);
        if (ShowDebug)
        {
		...
		trackerQuat.y = (float)rep.quat[0];	
		trackerQuat.z = (float)rep.quat[1];
...
*/		trackerQuat.x = -(float)rep.quat[0];	
	...
	{
		int num = MaxReports;
        VRPNTrackerPosReports("Rigid Body 1", repsPtr, ref num);
