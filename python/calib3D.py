                ...
                for calib_rnds in marker:
                    ...
                    for calib_rnd in calib_rnds:
                        ...
                        for datas in calib_rnd:
                            ...
                            for val in datas:
                                participant[layer_key][marker_key][calib_round_key][datas.tag].append(float(val.text))
                                #print(val.tag)
                                #print(val.attrib)
                                #print(val.text)
                                only_needed_data[layer_key][datas.tag].append(float(val.text))
                                if datas.tag == 'left_x':  # one time operation a hack
                                    only_needed_data[layer_key]['screen_x'].append(float(participant[layer_key][marker_key]['screen_x']))
                                    only_needed_data[layer_key]['screen_y'].append(float(participant[layer_key][marker_key]['screen_y']))
                                pass
...
#tree.write('Tim_left.xml', encoding="Windows-1252")
tree.write('Tim_left.xml')
print(".............")
